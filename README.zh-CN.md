...
- 结构体标签绑定 `FromStruct`：
    - 三种标签规则：`named`(默认) / `simple` / `field`(用字段名做选项名)；匿名嵌套结构体在任意规则下都会自动展开
...
- `gcli.TagRuleField`：用**字段名**(SnakeCase) 做选项名，元数据从独立 tag 键读取。

> **匿名嵌套结构体在三种规则下都会自动展开**——内部字段按当前生效的规则读取；内嵌*未导出*类型(如下方的 `commonOpts`)同样有效。
