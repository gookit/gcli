...
func TestCmdMarkdown_EnvDefaultDoesNotExposeValue(t *testing.T) {
	t.Setenv("SERVER_TOKEN", "real-secret-token")

	cmd := gcli.NewCommand("demo", "demo command", func(c *gcli.Command) {
		var token string
		c.StrOpt(&token, "token", "", "${SERVER_TOKEN}", "server token")
	})
	cmd.Init()

	md := docgen.CmdMarkdown(cmd)
	assert.StrContains(t, md, "${SERVER_TOKEN}")
	assert.NotContains(t, md, "real-secret-token")
}
