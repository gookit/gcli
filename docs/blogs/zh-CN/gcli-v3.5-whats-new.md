...
**动态补全（零维护）。** 默认生成的是一份*瘦*脚本：它不再硬编码名称，而是在补全时通过内置的 `--in-completion` 选项**回调你的二进制**来获取候选项。明天你新增一个命令，Tab 补全立即生效——无需重新生成。
...
候选计算逻辑有完整的单元测试覆盖；bash/zsh/pwsh 的 shell 胶水层都委托到这同一个动态入口，因此行为保持一致。

## 2. 命令中间件

想在命令主逻辑之前做鉴权、日志或计时——又不想把这些代码复制到每个命令里？中间件来了。

用 `Use()` 注册一个或多个处理器，它们会**按注册顺序、在**命令主函数 `Func` **之前**执行。任一处理器返回 error，链条即中止、错误向上传递（主函数 `Func` 被跳过）。
...
`Command.Use()` 和 `App.Use()` 都返回接收者本身，方便链式调用。未使用中间件的应用，行为与之前完全一致。

## 3. 帮助信息分组

应用变大后，扁平的命令/选项长列表会越来越难找。现在你可以用 `Category` 把它们归入带标题的分组。
...
分组按首次出现的顺序排列，组内条目按名称排序。当未设置分类时，输出与之前完全相同——完全向后兼容。
...
`field` 规则最为简洁：选项名来自字段名，而 `desc` / `default` / `required` 各自独立成键——易读也易维护。

## 5. 声明式交互收集

有时某个值是必需的，但用户忘了传。与其手写一个收集器，不如直接挂一个 `Question`：当选项值为空时，GCli 会交互式地提示输入（内置的默认收集器）。
...
经典的 POSIX 工具允许合并短选项：`-a -u -x` 可以写成 `-aux`。GCli 现在也支持了——而且很谨慎、**按需开启**，默认不改变任何行为。
...
关键的安全规则：只有当组合里**每个字符都是 bool 短选项**时才拆分。像 `-aO`（其中 `O` 需要取值）这样的混合写法会原样保留，绝不会误伤取值型短选项。（严格模式现在内部也走这条安全路径，取代了旧的"盲拆"逻辑。）
...
为什么移除 `--verbose`？它绑定的是一份应用内副本，而日志器从未读取它，因此毫无实际效果——只会让你应用的选项列表变得杂乱。请改用环境变量或代码来控制日志级别。
...
下的可运行示例：`struct-flag`（字段标签 + 匿名字段）、`short-merge`（EnhanceShort）、`ask-demo`（Question）。

## 结语

v3.5 的主题，就是让用 GCli 构建命令行应用变得更舒服——会自我维护的补全、承接那些"枯燥但必要"的横切逻辑的中间件、更整洁的帮助输出、更灵活的选项绑定——同时在底层悄悄修复了若干健壮性问题。
