...
4. pwsh：现仅 bash/zsh；PowerShell 模板作为可选增量（按需）。
...
2. `feat(completion): 运行期动态补全候选计算 + showAutoCompletion(--in-completion)`
3. `feat(completion): genac/动态脚本委托 --in-completion`
4. `feat(gflag): CliOpt.Choices 提供选项值候选`（可选）

> 进度：静态 `--gen-completion` 已落地（`gcli/completion.go` + `App.GenCompletionScript`）；
> 动态 `--in-completion`（`showAutoCompletion` 候选计算）仍待实现。
