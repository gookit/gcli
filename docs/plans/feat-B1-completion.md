...
4. ✅ `feat(gflag): CliOpt.Choices 提供选项值候选` — 已完成(commit 75bc031)
   - `CliOpt.Choices` + `WithChoices()` + `TakesValue()`；resolveCompletion 增加选项值补全分支。
5. ✅ PowerShell(pwsh) 动态补全（commit 49a6f6d）

> **B1 已全部完成**：静态 `--gen-completion`、动态 `--in-completion`、脚本委托动态 + 静默模式、
> pwsh 动态、选项值候选 `Choices` 均已落地并测试。
>
> 已知取舍（按需再加强）：补全不含隐藏命令的过滤（仅过滤了隐藏选项）；选项值词会停止命令下钻；
> pwsh/bash/zsh 的 shell 交互需人工 Tab 实测。
