...
1. `refactor(gflag): struct binding 去除 unsafe, 改用 Addr().Interface()`
2. `feat(gflag): struct tag 支持 slice([]string/[]int/[]bool) 与 time.Duration`
3. `feat(gflag): struct tag 支持 map[string]string(mapStrValue 适配器)`
4. `feat(gflag): struct tag 支持 enum 键(Choices + 成员校验)`
5. （可选/下一步）`feat(gflag): 泛型选项绑定 API Opt[T]/BindVar[T]`
