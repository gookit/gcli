...
- [x] A：`App` 复用 `gOpts`，删同步补丁，统一 `inCompletion`；补单测；全量测试通过
- [x] G：`findCommandName` 改 `foundCmd` 无副作用；补分支单测；全量回归通过
- [x] H：`helper`→`internal/helper`、删 `internal/help_tpl.go` 与 `gclicom/`；`go build ./... && go test` 全绿
- [ ] 在 CHANGELOG/README 注明：A 的「多 App 共享全局选项」、H 的「移除 helper/gclicom 公共包」（发版前补）
